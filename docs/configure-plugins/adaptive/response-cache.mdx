...
- Streaming calls are cached too. On a miss the live chunks are forwarded to
  ...
  event, and streams whose content cannot be replayed faithfully (including
...
| `key_strategy` | `"exact_request"` | `"exact_request"` reuses the same normalized request. `"logical"` additionally ignores tool-description wording and tool order while preserving the callable interface. |
